    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          required
        />
        <p class="text-gray-400 text-xs mt-1">URL: /cursos/{{ form.slug || 'slug-del-curso' }}</p>
      </div>
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2 cursor-pointer">
          <input v-model="form.is_published" type="checkbox" class="w-4 h-4 accent-indigo-600" />
          <span class="text-sm text-gray-700">Publicar curso</span>
        </label>

        <label class="flex items-center gap-2 cursor-pointer">
          <input v-model="form.is_featured" type="checkbox" class="w-4 h-4 accent-indigo-600" />
